...
namespace preemptive_executor {
    ...
    int CallbackRegistry::generate_threadgroup_id() {
    ...
    TimingExport CallbackRegistry::export_timing_information() {
        ...
        for (const auto& pair : callback_map_) {
            ...
            if (callback_info.threadgroup_id == 0) {
                throw std::runtime_error("Callback " + callback_info.callback_name +
                                         " has not been assigned to any threadgroup");
            }
    ...
    void CallbackRegistry::recursive_callback_traversal(
        const std::string& callback_name, int threadgroup_id, int prev_threadgroup_id,
        std::map<uint32_t, std::vector<int>>& deadline_to_threadgroup_id_map) {
        ...
        if (adjacency_list_[next_callback].indegree == 1 && outgoing.size() == 1) {
            ...
            return recursive_callback_traversal(next_callback, threadgroup_id, prev_threadgroup_id,
    ...
    void CallbackRegistry::recursive_threadgroup_traversal(int threadgroup_id) {
        ...
        if (threadgroup_adjacency_list_[threadgroup_id].indegree() == 0) {
            ...
            auto deadline =
                std::max_element(first_callback_info.deadlines.begin(), first_callback_info.deadlines.end());
    ...
    void CallbackRegistry::register_callback_entity(const CallbackEntity& entity,
                                                    rclcpp::CallbackGroup::SharedPtr group,
                                                    rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node) {
        const auto entity_it = pre_registered_names_.find(entity);
        if (entity_it == pre_registered_names_.end()) {
            std::cout << "Callback entity not pre-registered" << " Type=" << static_cast<int>(entity.get_type())
                      << std::endl;
            return;
        }
