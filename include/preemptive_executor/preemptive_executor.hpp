...
namespace preemptive_executor {
    struct ThreadGroupAttributes {
      public:
        ThreadGroupAttributes(int tg_id, int number_of_threads, int priority, bool is_mutex_group)
            : tg_id(tg_id), number_of_threads(number_of_threads), priority(priority), is_mutex_group(is_mutex_group) {}
        int tg_id;
    ...
    class PreemptiveExecutor : public rclcpp::Executor {
      public:
        ...
        RCLCPP_PUBLIC explicit PreemptiveExecutor(const rclcpp::ExecutorOptions& options,
      ...
      protected:
        ...
        memory_strategy::RTMemoryStrategy::SharedPtr rt_memory_strategy_ RCPPUTILS_TSA_PT_GUARDED_BY(mutex_);
