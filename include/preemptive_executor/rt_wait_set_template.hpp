...
#ifndef PREEMPTIVE_EXECUTOR__WAIT_SET_TEMPLATE_HPP_
#define PREEMPTIVE_EXECUTOR__WAIT_SET_TEMPLATE_HPP_
...
class RTWaitSetTemplate final : private SynchronizationPolicy, private StoragePolicy
{
public:
  RCLCPP_SMART_PTR_DEFINITIONS_NOT_COPYABLE(RTWaitSetTemplate)
  ...
  RCUTILS_WARN_UNUSED
  RTWaitResult<RTWaitSetTemplate>
  wait(std::chrono::duration<Rep, Period> time_to_wait = std::chrono::duration<Rep, Period>(-1))
  {
    ...
    // this method comes from the SynchronizationPolicy
    return this->template sync_wait<RTWaitResult<RTWaitSetTemplate>>(
...
private:
  // Add RTWaitResult type as a friend so it can call private methods for
  // acquiring and releasing resources as the RTWaitResult is initialized and
  // destructed, respectively.
  friend RTWaitResult<RTWaitSetTemplate>;

  /// Called by the RTWaitResult's constructor to place a hold on ownership and thread-safety.
  /**
  ...
  /// Called by the RTWaitResult's destructor to release resources.
  /**
...
#endif  // PREEMPTIVE_EXECUTOR__WAIT_SET_TEMPLATE_HPP_
